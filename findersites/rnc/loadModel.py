...
def loadModel():
    ################### DATASET HANDLING ####################
    DATASET_PATH = os.getcwd() + '\\findersites\\rnc\\'  # change the path to your dataset folder here
    ...
    def parseDataset():
        ...
        images = []
        for c in classes:
            images += ([os.path.join(DATASET_PATH + "clases", c, path) for path in
                        os.listdir(os.path.join(DATASET_PATH + "clases", c))])
