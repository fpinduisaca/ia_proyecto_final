...
import uuid
from .models import Imagen, Evento, EventoParada, EventoLimitado, UsuarioEventoParada, UsuarioEventoLimitado, \
    Recompensa, UsuarioRecompensa, Usuario
from django.contrib.auth.models import User
import json
from .rnc.recognizeImage import main
from .rnc.loadModel import loadModel
...
def mapa(request, info):
    ...
    if user != "undefined":
        ...
        for evento in eventosParada:
            distance = sqrt((float(evento.getPosition()['latitud']) - latitud) ** 2 + (
                        float(evento.getPosition()['longitud']) - longitud) ** 2)
            # Save nearby events
            if distance < 0.01:  # ~1km
                isVisited = UsuarioEventoParada.objects.filter(user_id=user_id,
                                                               event_id=evento.id)  # MODIFICAR USER_ID=1 POR USER_ID=REQUEST.USER.ID !!
                if isVisited:
        ...
        for evento in eventosLimitados:
            ...
            # Save nearby events
            if distance < 0.01:  # ~1km
                isVisited = UsuarioEventoLimitado.objects.filter(user_id=user_id,
                                                                 event_id=evento.id)  # MODIFICAR USER_ID=1 POR USER_ID=REQUEST.USER.ID !!
                if not isVisited:
                    prepareToSend.append(evento.returnMap())
...
def login(request):
    prepareToSend = []
    infoUser = json.loads(str(request.body))
    if not infoUser:
...
def register(request):
    datos = "false"
    infoUser = json.loads(str(request.body))
    if not infoUser or 'username' not in infoUser or 'password' not in infoUser or 'confirmPassword' not in infoUser or 'email' not in infoUser or \
            infoUser['password'] != infoUser['confirmPassword']:
        return HttpResponse(datos)

    try:
        User.objects.create_user(username=infoUser['username'], password=infoUser['password'],
                                 email=infoUser['email']).save()
        datos = "true"
    except:
        datos = "false"
