...
class UsuarioRecompensa(models.Model):
    ...
    key = models.CharField(validators=[RegexValidator(regex='^.{12}$', message='Length has to be 12', code='nomatch')],
                           max_length=12, verbose_name="Codigo", unique=True)
...
class EventoParada(models.Model):
    ...
    def returnJSON(self):
        ...
        return {'id': str(self.event.id), 'brand': self.event.brand, 'event_name': self.event.event_name,
                'event_description': self.event.event_description, 'event_type': 'parada',
                'latitud': self.event.latitud, 'longitud': self.event.longitud, 'reward': self.event.reward.reward_name,
                'cooldown': cooldown_formated}

    def returnMap(self):
        cooldown_formated = str(self.cooldown)
        return {'id': str(self.event.id), 'event_name': self.event.event_name.upper(), 'event_type': 'parada',
                'latitud': self.event.latitud, 'longitud': self.event.longitud, 'reward': self.event.reward.reward_name,
                'cooldown': cooldown_formated}
...
class EventoLimitado(models.Model):
    ...
    def returnJSON(self):
        ...
        return {'id': str(self.event.id), 'brand': self.event.brand, 'event_name': self.event.event_name,
                ...
                'event_fullDescription': self.event.event_fullDescription,
                'event_direccion': self.event.event_direccion, 'event_type': 'limitado', 'latitud': self.event.latitud,
                'longitud': self.event.longitud, 'reward': self.event.reward.reward_name, 'ini_date': ini_date_formated,
                'exp_date': exp_date_formated}
    ...
    def returnMap(self):
        ...
        return {'id': str(self.event.id), 'event_name': self.event.event_name.upper(), 'event_type': 'limitado',
                'latitud': self.event.latitud, 'longitud': self.event.longitud, 'reward': self.event.reward.reward_name,
                'ini_date': ini_date_formated, 'exp_date': exp_date_formated}
...
class UsuarioEventoParada(models.Model):
    ...
    def __str__(self):
        return self.user.username + ' - [' + str(
            self.event.event.id) + '] ' + self.event.event.brand + ': ' + self.event.event.event_name

    def returnJSON(self):
        last_use_formated = str(self.last_use).split("+")[0]
        cooldown_formated = str(self.event.cooldown)
        return {'id': str(self.event.event.id), 'brand': self.event.event.brand,
                'event_name': self.event.event.event_name, 'event_description': self.event.event.event_description,
                'event_type': 'parada', 'latitud': self.event.event.latitud, 'longitud': self.event.event.longitud,
                'reward': self.event.event.reward.reward_name, 'last_use': last_use_formated,
                'cooldown': cooldown_formated}

    def returnMap(self):
        last_use_formated = str(self.last_use).split("+")[0]
        cooldown_formated = str(self.event.cooldown)
        return {'id': str(self.event.event.id), 'event_name': self.event.event.event_name.upper(),
                'event_type': 'parada', 'latitud': self.event.event.latitud, 'longitud': self.event.event.longitud,
                'reward': self.event.event.reward.reward_name, 'last_use': last_use_formated,
                'cooldown': cooldown_formated}
